...
- No `旧品牌` strings remain in the project.
...
```bash
cd /Users/salt/Documents/个人自用/rimedou/旧品牌
...
- [ ] **Step 7: Verify no `旧品牌` strings in modified files**

```bash
grep -ri "旧品牌" . --exclude-dir=.git --exclude-dir=.build || true
```
...
Implementation details (Auto Layout constraints, visual style) follow the existing permission window but with rimedou branding. Do not include `旧品牌` strings.
