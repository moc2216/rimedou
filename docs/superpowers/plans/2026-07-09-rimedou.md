...
- No `DoubaoVoiceBridge` strings remain in the project.
...
```bash
cd /Users/salt/Documents/个人自用/rimedou/DoubaoVoiceBridge
...
- [ ] **Step 7: Verify no `DoubaoVoiceBridge` strings in modified files**

```bash
grep -ri "DoubaoVoiceBridge" . --exclude-dir=.git --exclude-dir=.build || true
```

Expected: no matches in `Package.swift`, `config.json`, `scripts/build-app.sh`, `support/Info.plist`.
...
Implementation details (Auto Layout constraints, visual style) follow the existing permission window but with rimedou branding. Do not include `DoubaoVoiceBridge` strings.
